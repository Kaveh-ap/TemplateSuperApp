...
import { getDevServerLocation } from '@callstack/repack/dist/modules/getDevServerLocation';
  ...
  async checkUpdates(update) {
    try {
      ...
      const unacceptedModules = updatedModules.filter(moduleId => {
        return renewedModules && renewedModules.indexOf(moduleId) < 0 && !moduleId.startsWith("webpack/container/entry");
      });
